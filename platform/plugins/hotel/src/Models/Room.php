...
class Room extends BaseModel
{
    ...
    public function isAvailableAt(array $filters = []): bool
    {
        ...
        if ($roomDates->isNotEmpty()) {
            foreach ($roomDates as $row) {
                if (! array_key_exists(date('Y-m-d H:i', strtotime($row->start_date)), $allDates)) {
                    continue;
                }
                ...
                $allDates[date('Y-m-d H:i', strtotime($row->start_date))] = [
        ...
        if ($roomBookings->isNotEmpty()) {
            foreach ($roomBookings as $roomBooking) {
                for (
                    ...
                    if (! array_key_exists(date('Y-m-d H:i', $index), $allDates)) {
                        continue;
                    }

                    $allDates[date('Y-m-d H:i', $index)]['number'] -= $roomBooking->number_of_rooms;

                    if ($allDates[date('Y-m-d H:i', $index)]['number'] <= 0) {
                        return false;
                    }
                }
            }
    ...
    public function getRoomTotalPrice(string $startDate, string $endDate, ?int $rooms = 1): float|int
    {
        ...
        $allDates = [];
        for ($index = strtotime($startDate); $index < strtotime($endDate); $index += 3600) {
            $allDates[date('Y-m-d H:i', $index)] = [
                'number' => $this->number_of_rooms,
                'price' => $this->price,
            ];
